...
export async function getCurrDelta(wholeSent = false) {
  ...
  if (wholeSent) { // estimate whole sentence duration in seconds
    ...
    if (currSubIndex == subsCount - 1) return 60;
  ...
  if (wholeSent) {
    ...
    console.log('currSub:', currSubIndex, 'words', words.length, 
...
async function estimateSecondsPerWord(index) {
  ...
  if (b >= e) { return secondsPerWord; }

  let avgWordDuration = (e - b) / q.split(/\s+/).length;
  secondsPerWord = (avgWordDuration <= maxSecondsPerWord) ? 
    avgWordDuration : secondsPerWord;
